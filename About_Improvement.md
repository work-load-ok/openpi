...
* CustomLeRobotDataset  # advantage训练选择
    ...
    * 增加自定义处理需求
        * 自定义参数
            ...
            - ~~`timestep_difference_mode`: 时间步差异模式，用于对比学习 # 默认true~~
            - ~~`stage_process_mode`: 阶段进度监督模式 # 默认true~~
        * 核心方法
...
## transforms.py
* `TokenizePrompt`类改造 # advantage全删
...
* 新增`Pi0Config_Custom`类（继承Pi0Config）
    * 自定义参数
        - ~~`timestep_difference_mode`: 时间步差异模式 # 内部默认执行~~
        - ~~`stage_process_mode`: 阶段进度模式 # 默认执行~~
        - ~~`with_value_head`: 是否启用value head # 默认执行~~
...
* 新增`PI0Pytorch_Custom`类（继承PI0Pytorch）
    * 配置参数
        ...
        - ~~`timestep_difference_mode`: 时间步差异模式 # 默认执行~~
    ...
    * 改造方法
        ...
        - ~~`embed_prefix()`: action_advantage通过sincos位置编码嵌入，拼接到lang_emb # advantage删~~
        - ~~`embed_suffix()`: 支持训练时按概率mask ego state（置0）# mask用不到~~
